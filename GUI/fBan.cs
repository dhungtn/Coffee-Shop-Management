...
namespace GUI
{
    public partial class fBan : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void LoadTable()
        {
            try
            {
                gcTable.DataSource = BanBUS.Instance.GetAllTable();
        ...
        mark:
            ...
            if (BanBUS.Instance.InsertTable(table))
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                LoadTable();
                SplashScreenManager.CloseForm();
                ...
                NhatKy.WriteLog("add new Table, ID = " + tableID);
            }
            else
        ...
        private void btnRemove_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                if (BanBUS.Instance.DeleteTable(id))
                {
                    LoadTable();
                    SplashScreenManager.CloseForm();
                    XtraMessageBox.Show("Đã xóa " + name, "Thông báo");
                    NhatKy.WriteLog("delete Table, ID = " + id);
                }
                else
                {
                    SplashScreenManager.CloseForm();
        ...
        private void gvTable_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            if (BanBUS.Instance.UpdateTable(id, name))
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                LoadTable();
                SplashScreenManager.CloseForm();
                NhatKy.WriteLog("update Table: " + curTable + " to " + name);
            }
            else
            {
                SplashScreenManager.CloseForm();
