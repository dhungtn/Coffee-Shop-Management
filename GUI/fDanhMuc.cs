...
namespace GUI
{
    public partial class fDanhMuc : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void LoadCategory()
        {
            try
            {
                gcCategory.DataSource = DanhMucBUS.Instance.GetAllCategory();
        ...
        private void InsertCategory(GridView view, int rowHandle)
        {
            ...
            if (DanhMucBUS.Instance.InsertCategory(name))
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                LoadCategory();
                SplashScreenManager.CloseForm();
                NhatKy.WriteLog("add new Category: " + name);
            }
            else
        ...
        private void UpdateCategory(GridView view, int rowHandle)
        {
            ...
            if (DanhMucBUS.Instance.UpdateCategory(int.Parse(id), name))
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                LoadCategory();
                SplashScreenManager.CloseForm();
                NhatKy.WriteLog("update Category: " + curName + " -> " + name);
            }
            else
        ...
        private void btnRemove_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                if (DanhMucBUS.Instance.DeteleCategory(int.Parse(id)))
                {
                    SplashScreenManager.ShowForm(typeof(WaitForm1));
                    LoadCategory();
                    SplashScreenManager.CloseForm();
                    XtraMessageBox.Show("Xóa danh mục thành công", "Thông báo");
                    NhatKy.WriteLog("delete Category: " + name);
                }
                else
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                gcCategory.DataSource = DanhMucBUS.Instance.SearchCategoryByName(txtSearchCategory.Text);
                SplashScreenManager.CloseForm();
            }
            catch (Exception ex)
            {
