...
namespace GUI
{
    public partial class fDoAn : DevExpress.XtraEditors.XtraForm
    {
        DoAn curFood;
        ...
        private void LoadFoodToGridControl()
        {
            try
            {
                gcFood.DataSource = DoAnBUS.Instance.GetAllFood();
            ...
            try
            {
                myLookup.DataSource = DanhMucBUS.Instance.GetAllCategory();
        ...
        private void AddFood(GridView view, int rowHandle)
        {
            ...
            DoAn newFood = new DoAn(name, int.Parse(categoryID), price);
            if (DoAnBUS.Instance.InsertFood(newFood))
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                LoadFoodToGridControl();
                SplashScreenManager.CloseForm();
                NhatKy.WriteLog("add new Food: " + name);
            }
            else
            {
                SplashScreenManager.CloseForm();
        ...
        private void UpdateFood(GridView view, int rowHandle)
        {
            ...
            SplashScreenManager.ShowForm(typeof(WaitForm1));
            DoAn food = new DoAn(int.Parse(id), name, int.Parse(categoryID), price);
            if (DoAnBUS.Instance.UpdateFood(food))
            {
                LoadFoodToGridControl();
                SplashScreenManager.CloseForm();
                NhatKy.WriteLog("update Food: Name: " + curFood.Name + " -> " + name
        ...
        private void btnRemove_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                if (DoAnBUS.Instance.DeleteFood(id))
                {
                    SplashScreenManager.ShowForm(typeof(WaitForm1));
                    LoadFoodToGridControl();
                    SplashScreenManager.CloseForm();
                    XtraMessageBox.Show("Đã xóa " + name, "Thông báo");
                    NhatKy.WriteLog("delete Food: " + name);
                }
                else
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                gcFood.DataSource = DoAnBUS.Instance.SearchFoodByName(txtSearchFood.Text);
                SplashScreenManager.CloseForm();
            }
            catch (Exception ex)
            {
        ...
        private void gvFood_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            curFood = new DoAn(name.ToString(), (int)categoryID, (int)price);
        }
    }
}