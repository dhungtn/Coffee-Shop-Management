...
namespace GUI
{
    public partial class fHoaDon : DevExpress.XtraEditors.XtraForm
    {
        ...
        void LoadListBillByDate(DateTime fromDate, DateTime toDate)
        {
            try
            {
                gcBill.DataSource = HoaDonBUS.Instance.GetListBillByDate(fromDate, toDate);
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            if (XtraMessageBox.Show("Xóa hóa đơn " + id + "?", "Xác nhận", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    ThongTinHoaDonBUS.Instance.DeleteBillInfoByBillID(id);
                }
                catch (Exception ex)
                {
                    XtraMessageBox.Show("Error: " + ex);
