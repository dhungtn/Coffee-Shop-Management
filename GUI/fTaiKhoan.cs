...
namespace GUI
{
    public partial class fTaiKhoan : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void LoadAcount()
        {
            try
            {
                gcAccount.DataSource = TaiKhoanBUS.Instance.GetAllAcount();
            ...
            try
            {
                myLookup.DataSource = KieuTaiKhoanBUS.Instance.GetAllAccountType();
        ...
        private void btnRemove_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                if (TaiKhoanBUS.Instance.Delete(userName))
                {
                    SplashScreenManager.ShowForm(typeof(WaitForm1));
                    LoadAcount();
                    SplashScreenManager.CloseForm();
                    XtraMessageBox.Show("Xóa tài khoản thành công");
                    NhatKy.WriteLog("delete Account: " + userName);
                }
                else
        ...
        private void btnResetPassword_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (XtraMessageBox.Show("Đặt lại mật khẩu cho tài khoản " + userName + "?", "Xác nhận", MessageBoxButtons.YesNo)
                == DialogResult.Yes)
            {
                if (TaiKhoanBUS.Instance.ResetPassword(userName))
                {
                    SplashScreenManager.ShowForm(typeof(WaitForm1));
                    LoadAcount();
                    SplashScreenManager.CloseForm();
                    XtraMessageBox.Show("Đặt lại mật khẩu thành công\n Mật khẩu mặc định là '0'");
                    NhatKy.WriteLog("set password for Account: " + userName);
                }
                else
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                SplashScreenManager.ShowForm(typeof(WaitForm1));
                gcAccount.DataSource = TaiKhoanBUS.Instance.SearchAccountByUserName(txtSearchAccount.Text);
                SplashScreenManager.CloseForm();
            }
            catch (Exception ex)
            {
